...
export const page$_ = be_<page$_T>('page$', ctx=>{
	...
	const [host$, set_host] = split_atom_<string|undefined>(undefined)
	host$.subscribe(host=>event_log$.add({ host }))
	const [path$, set_path] = split_atom_<string|undefined>(undefined)
	path$.subscribe(path=>event_log$.add({ path }))
	const [params$, set_params] = split_atom_<PageParams|undefined>(undefined)
	params$.subscribe(params=>event_log$.add({ params }))
	const [query$, set_query] = split_atom_<Query|undefined>(undefined)
	query$.subscribe(query=>event_log$.add({ query }))
	const [error$, set_error] = split_atom_<Error|undefined>(undefined)
	error$.subscribe(error=>event_log$.add({ error }))
	const page$ = atom_<PageContext|null>(null)
	page$.subscribe(page=>{
		event_log$.add({ page })
		set_host(page?.host)
		set_path(page?.path)
...
export interface page$_T extends WritableAtom_<PageContext> {
	host$:page_host$_T
	path$:page_path$_T
